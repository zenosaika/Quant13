...
async function handleDiscordPrompt(
  ...
  if (!webhookUrl) {
    addLog({
      agent: 'system',
      headline: '⚠️  Discord not configured',
  ...
  if (!result.results_path) {
    addLog({
      agent: 'system',
      headline: '⚠️  Results path missing',
  ...
  if (!isInteractiveSession) {
    addLog({
      agent: 'system',
      headline: 'ℹ️  Discord auto-send disabled',
  ...
  const promptLogId = addLog({
    agent: 'system',
    headline: '📡 Discord Integration Available',
  ...
  if (userChoice) {
    ...
    try {
      ...
      updateLog(promptLogId, {
        headline: '✅ Discord transmission complete',
        content: `Report sent to Discord successfully!`,
        status: 'completed',
      });
    } catch (error) {
      updateLog(promptLogId, {
        headline: '❌ Discord transmission failed',
